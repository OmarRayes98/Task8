...
const Products = () => {
  ...
  useEffect(() => {
    ...
    else dispatch(searchProducts("")); // for reset ,  get all data
  }, [dispatch, debouncedQuerySearch]);

  const totalPages = Math.ceil(filterProducts?.length / limit);

  const currentPageItems = isArray(filterProducts)
    ...
    <section className=" px-[64px] md:px-[109px] pt-[24px] pb-[24px]">
      <div className="relative w-full">
        ...
        <input
          ...
          className="block w-full h-10 px-4 rounded-lg text-sm font-normal border border-input outline-none  focus:border-primary/90"
